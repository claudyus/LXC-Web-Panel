...
class TestMockLxc(unittest.TestCase):
    ...
    def test_01_config(self):
        shutil.rmtree('/tmp/lxc', ignore_errors=True)
        out = subprocess.check_output('lxc-config lxc.lxcpath', shell=True).strip()
        assert out == '/tmp/lxc'

    def test_02_create(self):
        subprocess.check_output('lxc-create -n lxctest', shell=True).strip()
    ...
    def test_03_start(self):
        subprocess.check_output('lxc-start -dn lxctest', shell=True).strip()
        with open('/tmp/lxc/lxctest/status') as f:
            content = f.readlines()
            assert 'RUNNING' in content[0].rstrip('\n')

    def test_04_ls(self):
        out = subprocess.check_output('lxc-ls --fancy | grep test', shell=True).strip()
        assert 'lxctest' in out

    def test_05_stop(self):
        subprocess.check_output('lxc-stop -n lxctest', shell=True).strip()
        with open('/tmp/lxc/lxctest/status') as f:
            content = f.readlines()
            assert 'STOPPED' in content[0].rstrip('\n')

    def test_06_destroy(self):
        subprocess.check_output('lxc-destroy -n lxctest', shell=True).strip()
